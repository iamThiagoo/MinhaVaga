...
<x-app-layout>
    <section class="container flex-col justify-center flex-1 w-full mx-auto my-10 lg:flex min-h-3/4">
        ...
        <div class="flex flex-col justify-center gap-10 px-5 mt-3 lg:px-0 lg:gap-20 lg:flex-row">
            <form class="w-full my-3 lg:w-4/12" enctype="multipart/form-data">
                @csrf
                <div class="">
                        ...
                        @foreach($experiences as $experience)
                            <div class="experience-card my-5 border-b-2 pb-5">
                                <div class="flex items-start gap-3 mb-4 lg:mb-2">
                                    ...
                                    <div>
                                        <h2 class="font-black text-xl mb-1">{{ $experience->name }} em <span class="text-sky-900"> {{ $experience->company }} </span> </h2>
                                        <h3 class="text-base mb-2">
                                            {{ date('M/Y', strtotime($experience->initial_date)) }} - {{ date('M/Y', strtotime($experience->final_date)) }}
                                        </h3>
                                    </div>
                                </div>
                                <div>
                                    <p class="px-2"> {{ $experience->details }} </p>
                                    ...
                                    <div class="flex justify-end mt-4 gap-2 pr-2">
                                        <form method="POST" action="{{ route('experience.destroy', $experience->id) }}">
                                            ...
                                            <button type="submit" class="bg-red-500 text-white gap-2 p-2 px-4 flex items-center rounded-lg lg:text-sm hover:opacity-95">
                                                Excluir
